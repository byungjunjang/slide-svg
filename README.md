...
표지·인포그래픽에 AI 이미지를 쓰고 싶을 때만 필요합니다. 환경변수 1개만 채우면 자동 활성화:

```bash
cp .env.example .env
# .env 파일에 IMAGE_BACKEND=gemini|openai 와 해당 API 키를 채워넣음
```

이미지 백엔드가 없으면 슬라이드는 **네이티브 도형 + SVG 일러스트**로만 생성됩니다 (Jangpm 기본 동작).
...
4. **(선택) Image Generator** — 표지·인포그래픽용 AI 이미지 생성 (이미지 백엔드 설정 시)
...
```bash
pip install -r .claude/skills/slide/requirements.txt        # 의존성 설치 (한 번만)
...
| **[slide-pencil](https://github.com/byungjunjang/slide-pencil)** | 자연어 → Pencil → React/Tailwind → HTML/PPTX/PDF | 디자인 일관성, 시각 레퍼런스 기반, 토큰 컨트랙트 |
...
Step 4: (선택) Image Generator (image_gen.py)
  ├─ IMAGE_BACKEND=gemini|openai
  ├─ 표지·인포그래픽 일러스트 생성
  └─ 워터마크 제거 (gemini_watermark_remover.py)
...
- **images**: `<image>` 태그로 PPTX에 임베드. AI 이미지는 `image_gen.py` 출력을 참조
...
Pillow, numpy               ← 이미지 처리
google-genai, openai        ← AI 이미지 백엔드 (선택)
jsonschema                  ← /theme-init 토큰 검증
pytest                      ← 회귀 테스트
```
