...
### 4단계. AI 이미지 생성 — 호스트별 백엔드

표지·인포그래픽에 AI 이미지가 필요할 때 `/slide` Step 5(Image_Generator)가 호스트별 승인 백엔드를 자동으로 사용합니다.

- **Claude Code**: vendored `/codex-image` 스킬이 **API 키 발급·관리 없이 Codex CLI OAuth(ChatGPT 로그인)만으로** `gpt-image-2`를 호출합니다. 아래 "최초 1회 준비"가 필요합니다.
- **Codex**: 런타임 기본 `imagegen` 스킬 / 내장 `image_gen` 도구를 사용합니다 — 별도 설치·로그인 불필요. (`.codex/skills`에는 `codex-image`를 패키징하지 않습니다.)

**최초 1회 준비 — Claude Code `/codex-image` 경로 전용 (한 번만 하면 됩니다):**
...
이미지 백엔드 preflight 또는 실행이 실패하면 **AI 이미지 슬롯이 필요한 덱은 HALT**합니다. Claude Code에서는 `/codex-image` 로그인 상태를 확인하고, Codex에서는 `imagegen` / `image_gen` 가용성을 Image_Generator 실행 시점에 확인합니다. 이미지 슬롯이 없는 덱만 네이티브 도형 + SVG 다이어그램으로 계속 진행할 수 있습니다. placeholder, 단색 이미지, 다른 이미지 백엔드로 silent fallback 하지 않습니다.
...
4. **(선택) Image Generator** — 표지·인포그래픽용 AI 이미지 생성. Claude Code는 `/codex-image`, Codex는 기본 `imagegen` / `image_gen`을 사용합니다 — 셋업은 위 "4단계" 참조.
...
**Q. 기밀 데이터인데 안전한가요?**
A. 파일은 모두 로컬에서 처리됩니다. 단, Claude와의 대화 내용은 AI 응답을 받기 위해 Anthropic 서버로 전달됩니다. 회사 보안 정책에 따라 판단하세요. AI 이미지 생성은 호스트별 승인 경로만 사용합니다: Claude Code는 `/codex-image`, Codex는 기본 `imagegen` / `image_gen`. 다른 백엔드(Gemini/DALL·E/Midjourney 등)는 사용하지 않습니다.
