...
Take the Strategist's Design Spec + Content Outline and generate SVG pages page-by-page into `<project_path>/svg_output/`, then write speaker notes to `<project_path>/notes/total.md`. Every page follows the active-theme ({{TOKEN:display_name}}) visual language: monochrome + single `{{TOKEN:colors.accent}}` accent, font chain `{{TOKEN:typography.font-chain}}`, 1280×720, {{TOKEN:voice.tone}} tone.
...
✅ Color: monochrome + {{TOKEN:colors.accent}} accent (max 1–2 accent events per slide)
✅ Typography: active-theme weights {{TOKEN:typography.body.weight}}/{{TOKEN:typography.caption.weight}}/{{TOKEN:typography.title.weight}}/{{TOKEN:typography.headline.weight}}/{{TOKEN:typography.display.weight}}
✅ Body baseline: {{TOKEN:typography.body.size}}px (active-theme standard) — or {{TOKEN:typography.title.size}}px for relaxed density
✅ Font family chain: {{TOKEN:typography.font-chain}}
✅ GM line: present on every content slide, y=680, {{TOKEN:typography.body.size}}px / 700 / {{TOKEN:colors.text-secondary}} / centered
```
...
- **Accent > 2** (excluding chart ladders): downgrade the least semantically loaded accent use to `{{TOKEN:colors.text}}` (text) or `{{TOKEN:colors.border}}` (border).
...
| Report list with rule lines | Text-primary slides (anti Rule 15) | Stack items with `<line stroke="{{TOKEN:colors.border}}" stroke-width="1"/>` between |
...
- Tertiary: 0.4
- Quaternary: 0.25
- Grid / axis: `{{TOKEN:colors.border}}`
- Axis labels: `{{TOKEN:colors.text-secondary}}` {{TOKEN:typography.caption.size}}px
- Chart title (if separate from headline): `{{TOKEN:colors.text}}` {{TOKEN:typography.title.size}}px 600
...
ls .claude/skills/slide/templates/icons/tabler-outline/
```
...
3. **No gradient text** — fills are solid `{{TOKEN:colors.text}}` or `{{TOKEN:colors.accent}}` only
...
- Style: `font-family` = full active-theme font chain, `font-size="{{TOKEN:typography.body.size}}"`, `font-weight="700"`, `fill="{{TOKEN:colors.text-secondary}}"`
...
```
python3 .claude/skills/chart-design/scripts/render_chart.py <spec.json> -o <frag.svg> --pos X,Y
```

The engine computes scales/ticks/arcs from the numbers and resolves every color/font from the active theme tokens — hand-adapted geometry is where mis-scaled bars and wrong percentages come from. Keep each spec JSON next to the project (specs are the source of truth; re-render after a theme swap). Selection judgment, spec schema, and the embedding contract: `.claude/skills/chart-design/SKILL.md` + its `references/spec-format.md` / `references/integration.md` (includes the `chart_strategy` → type map for plan-consuming mode).
...
<rect x="56" y="180" width="560" height="315"
      fill="{{TOKEN:colors.surface-alt}}" stroke="{{TOKEN:colors.border}}" stroke-width="1" stroke-dasharray="8,4"/>
<text x="336" y="340" text-anchor="middle"
      font-family="{{TOKEN:typography.font-chain}}" font-size="{{TOKEN:typography.body.size}}" fill="{{TOKEN:colors.text-tertiary}}">
...
```xml
<text x="56" y="96"
      font-family="{{TOKEN:typography.font-chain}}"
...
- Accent: `{{TOKEN:colors.accent}}` — use sparingly (title emphasis, hero stat)

### Never

- Pure black `#000000` (use `{{TOKEN:colors.text}}`)
...
- [ ] Grid spacing is 8-multiple; card radius {{TOKEN:radius.md}}px; borders `1px solid {{TOKEN:colors.border}}`
