...
## Text Width Estimation and Viewer Wrap Behavior (Hangul note)

DrawingML `<a:bodyPr wrap="none"/>` asks the viewer to render text on a single line, but mobile and lightweight PPTX viewers often honor the shape bounds (`<a:ext cx="..."/>`) first and force-wrap anyway when a text run exceeds the bounds. So the converter must size text box width with enough headroom that those viewers don't feel compelled to wrap.

Width is estimated by `scripts/svg_to_pptx/drawingml_utils.py::estimate_text_width()`, which consults `is_cjk_char()` to decide whether to charge a full `font_size` (CJK) or a narrower Latin default (`font_size × 0.55`). The unicode block set in `is_cjk_char` must include every ideographic / syllabic script you actually generate — notably **Hangul Syllables (U+AC00–U+D7AF)** and **Jamo / Compatibility Jamo**, which live outside the "CJK Unified Ideographs" blocks and are easy to miss. Missing them silently undersizes Korean text boxes by roughly 45%, triggering forced wrap in mobile viewers even though the SVG source is correct.

The current converter also applies a 1.25× headroom factor on top of the estimate (see `drawingml_elements.py`) as a defensive margin for viewer rendering variance. Lower values reintroduce wrap regressions on some mobile clients.

Related: `docs/solutions/bug-fix/sol-20260423-001.md`, `docs/solutions/workflow/sol-20260423-003.md`.
