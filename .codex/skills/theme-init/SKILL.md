...
`.codex/skills/slide/templates/layouts/<theme.name>/DESIGN.md`. Token-driven
...
`.codex/skills/slide/templates/layouts/jangpm/DESIGN.md` is hand-authored
...
> `pip install cairosvg` (or `pip install -r .codex/skills/slide/requirements.txt`).
...
```bash
python3 .codex/skills/theme-init/scripts/render_layouts.py     # now picks up _shell_src/
python3 .codex/skills/theme-init/scripts/validate_shells.py    # FATAL lock check
python3 .codex/skills/theme-init/scripts/preview_shells.py     # build _preview/index.html
```
...
2. **Gallery check.**
   ```bash
   python3 -m http.server -d .codex/skills/slide/references/jangpm-patterns 8000
   ```
...
1. Present the preview to the user. Point them at it:
   ```bash
   python3 -m http.server -d .codex/skills/slide/templates/layouts/<name>/_preview 8000
...
- `jsonschema>=4.17.0` (listed in `.codex/skills/slide/requirements.txt`)
...
| Path | Role |
|------|------|
| `.codex/skills/slide/references/theme-active.json` | Source of truth for all downstream renders |
| `.codex/skills/slide/references/design-system.md` | Human-readable design token reference (rendered) |
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `jsonschema missing` | Deps not installed | `pip install -r .codex/skills/slide/requirements.txt` |
...
to repo root, e.g.,
`.codex/skills/slide/templates/layouts/<theme>/assets/brand/<image>.png`)
...
`.codex/skills/slide/references/theme-active.json` (Jangpm) for a fully
populated theme.
...
init_theme; rollback is `git log -- .codex/skills/slide/references/theme-active.json`
