...
> flow (`references/onboarding.md`) or the §0 style-guide gate; the style guide is pre-skinned to the
> active theme. Output target for decks is native DrawingML SVG, not HTML. The HTML-centric guidance
> below is upstream context — read it for layout/type conventions, then emit per `diagram-types.md` §3.
