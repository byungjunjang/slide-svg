...
```bash
python3 -c "import json; t=json.load(open('.codex/skills/slide/references/theme-active.json')); print(f'[active theme] {t[\"display_name\"]} ({t[\"name\"]}) — accent {t[\"colors\"][\"accent\"]}, font {t[\"typography\"][\"font-chain\"].split(\",\")[0]}')"
```
...
1. **Validate the plan first.** Re-run the plan validator to catch any post-edit drift:
   ```bash
   .codex/skills/slide/scripts/_py.sh .codex/skills/slide-plan/scripts/validate_plan.py <project_path>/slide_plan.json
   ```
