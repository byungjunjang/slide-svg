...
| Data-driven chart renderer | `${REPO_ROOT}/.codex/skills/chart-design/` | 21 quantitative chart types rendered from data spec JSONs (`scripts/render_chart.py`); geometry computed, style resolved from active theme tokens. Executor uses this instead of hand-drawing whenever a slide charts real numbers (executor.md §7) |
...
> The schema this file conforms to (v1 token contract) lives at `.codex/skills/theme-init/references/token-contract.json`. Read that if you need to know which fields are required, what value formats are accepted, or how to validate a hand-edited theme.

```bash
python3 .codex/skills/slide/scripts/announce_theme.py
```
