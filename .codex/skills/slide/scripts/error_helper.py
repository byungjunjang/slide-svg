#!/usr/bin/env python3
"""Validation error catalog with actionable fix suggestions.

Scope: pre-flight project-validation errors only (missing files, malformed
naming, viewBox mismatch, etc.) — NOT a general logging layer. Consumed by
`project_utils.py` (validate command) and `svg_quality_checker.py`. The rest
of the script tree uses bare `print()` for progress output, which is the
